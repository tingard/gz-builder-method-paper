...
\comment{Sandor suggests replacing this list with a table with selection name (i.e. ''low z``), expression ($0.02 < z < 0.055$), number of galaxies remaining}
...
\begin{itemize}
  ...
  \item Computation of stellar mass completeness limits using the method of \citet{Pozzetti2009:0907.5416v2} and limits calculated by \citet{2017MNRAS.472.2263H}, namely: \\
    \indent$9.45 < \log(M_* / M_\odot) \le 11.05$\\
    \indent$\log({M_* / M_\odot}) > 2.07\log(z) + 12.64$\\
    \indent$\log({M_* / M_\odot}) < 2.45\log(z) + 14.05$.
\end{itemize}
...
\begin{figure*}
  ...
  \caption{Components drawn by volunteers for UGC 4721. The top left panel shows drawn discs (blue), top right shows drawn bulges (orange), bottom left shows drawn bars (green) and bottom right shows drawn spiral arms. \comment{Add component numbers to panels}}
