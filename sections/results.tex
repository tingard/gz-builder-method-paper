...
\comment{Brooke - With a tool like galfit it really depends on the spiral arms, but it tends to lead to oversubtraction of other parts of the disk, and a slight change of ellipticity. Unless the arms are really strong and/or the disk is low S/N, the disk just has a lot of pixels that tend to keep the major parameters close to the ''real`` value, because galfit's first priority always seems to be conserving flux and those disk pixels add up. But with a grand design spiral or similar all bets may be off}
